...
| `npm test` | 597 tests: the HubSpot contract gate, the v1 import, per-block snapshots in every theme, the linter, branch coverage, migrations, editing, the design system, multi-column rows and groups, the paste sanitiser, local assets, the verified HubSpot facts, and the purity checks |
