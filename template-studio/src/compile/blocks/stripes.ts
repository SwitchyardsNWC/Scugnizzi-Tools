...
import type { Section, StripesBlock } from '../../model/types.ts';
import { colorOf } from '../../model/design-system.ts';

export function renderStripes(block: StripesBlock, sec: Pick<Section, 'bleed'>, ctx: BuildContext): IRNode {
  const bands = block.stripes
    .map((s) => ({ height: s.height, band: colorOf(ctx.ds, s.color) }))
    .filter((s) => s.height > 0 && s.band)
    .map((s) => section(spacerRow(s.height), { ds: ctx.ds, band: s.band, bleed: sec.bleed ?? false }));
  return frag(bands);
}
